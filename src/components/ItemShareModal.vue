...
import { fetchImageAsDataUrlForShare } from '../utils/imageUtils';
...
/** data:image/... after fetch — never use raw API URL in the card (avoids CORS + @error races). */
const shareImageDataUrl = ref(null);

const displayShareSrc = computed(() => shareImageDataUrl.value);
...
watch(
  () => props.imageUrl,
  async (url) => {
    shareImageDataUrl.value = null;
    imgLoadError.value = false;
    hidePhotoForExport.value = false;
    if (!url) return;
    const dataUrl = await fetchImageAsDataUrlForShare(url);
    shareImageDataUrl.value = dataUrl;
    if (dataUrl) imgLoadError.value = false;
  },
  { immediate: true }
);
